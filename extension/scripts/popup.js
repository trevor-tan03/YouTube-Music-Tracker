...
function renderCurrentVideo(video) {
    ...
    if (!video) {
        container.innerHTML = '<div class="none">Not playing</div>';
        return;
    }
    container.innerHTML = `
        <img class="now-thumb" src="${video.thumbnailUrl[0]}" alt="thumbnail of current video" width="80" height="45" />
        <div class="now-info">
            <div class="now-title">${video.title}</div>
            <div class="now-channel">${video.channel}</div>
        </div>
    `;
}
...
document.addEventListener("DOMContentLoaded", () => {
    if (chrome.storage.session) {
        chrome.storage.session.get("currentVideo").then(({ currentVideo }) => {
